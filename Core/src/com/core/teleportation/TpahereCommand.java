...
import com.core.utils.RequestType;
...
public class TpahereCommand implements CommandExecutor {
	...
	public TpahereCommand(Essentials e) {
		essentials = e;
	}
		...
		if (args.length == 0) {
			...
			if (target != null) {
				if (!TptoggleUtil.isDenying(target)) {
					TeleportRequest request = new TeleportRequest(player,
							target, RequestType.TPAHERE);
					request.getSender()
							.sendMessage(
									essentials.prefix
											+ "§aTeleport request successfully sent to §e"
