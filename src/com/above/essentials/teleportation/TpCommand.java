		...
		if (args.length == 0) {
			...
			if (target != null) {
				if (!TptoggleUtil.isDenying(target)) {
					...
					player.sendMessage(essentials.prefix + "§aSuccessfully teleported to §e" + target.getName());
					target.sendMessage(essentials.prefix + "§e" + player.getName() + "§a has teleported to you");
				} else {
